# 60 days of Claude AI Challenge - Day 29

# Operation Lifeline: Supply Chain Crisis Lab


**Project:** Operation Lifeline: Supply Chain Crisis Lab

An interactive single-file React simulation where players become the Chief Supply Chain Officer of a fictional enterprise and navigate a major supply chain disruption through strategic decision-making, supplier negotiations, executive leadership, AI investments, and performance analysis.


# Project Overview

Operation Lifeline is designed to make supply chain concepts accessible to complete beginners while still presenting realistic business trade-offs.

Players progress through seven stages:

1. Company Profile
2. Crisis Detection
3. War Room Strategy
4. Supplier Negotiation
5. CEO Boardroom
6. AI Strategy
7. Crisis Debrief Dashboard

Every playthrough generates a different company, crisis scenario, business metrics, and final outcome.


# Features

- Random fictional company generation
- Multiple crisis scenarios
- Interactive war room decision making
- Animated business KPI dashboard
- Supplier negotiation simulator
- CEO leadership assessment
- AI investment planning
- Executive performance dashboard
- Personalized recommendations
- Fully responsive enterprise UI
- Offline single HTML application


# Technology Used

- HTML5
- CSS3
- JavaScript
- React (CDN)
- Babel JSX

No backend, APIs, databases, or external assets were used.


# Screenshots

### 1. Welcome Screen

![Welcome Screen](https://github.com/ridak5845/60-days-claude-journey/blob/main/Day-29/screenshot-1782730315700.png)

Introduces the simulation, objectives, and seven-stage workflow.


### 2. Company Profile

![Company Profile](https://github.com/ridak5845/60-days-claude-journey/blob/main/Day-29/Screenshot_29-6-2026_162229_.jpeg)

Randomly generated company including:

- Industry
- Revenue
- Warehouses
- Suppliers
- Inventory Days
- Lead Time
- Countries
- Risk Rating


### 3. Crisis Detection

![Crisis Screen](https://github.com/ridak5845/60-days-claude-journey/blob/main/Day-29/Screenshot_29-6-2026_16232_.jpeg)

Example crisis:

**Supplier Bankruptcy**

Displays:

- Urgency
- Financial Impact
- Operations Affected
- Why it matters
- Next steps


### 4. War Room

![War Room](https://github.com/ridak5845/60-days-claude-journey/blob/main/Day-29/Screenshot_29-6-2026_162435_.jpeg)

Choose three strategic responses from six available actions.

Business metrics update dynamically:

- Cost
- Inventory
- Profit
- Delivery Speed
- Customer Satisfaction


### 5. Supplier Negotiation

![Negotiation](screenshots/negotiation.png)

Four negotiation rounds where each decision changes:

- Trust
- Price
- Lead Time

A final negotiation score is calculated.


### 6. CEO Boardroom

![CEO Dashboard](screenshots/ceo.png)

Five executive-level leadership questions evaluate strategic thinking and crisis management.


### 7. AI Strategy

![AI Strategy](screenshots/ai-strategy.png)

Select two AI investments:

- Demand Forecasting
- Inventory Optimization
- Supplier Risk Monitoring
- Warehouse Vision
- Procurement Copilot

Each investment changes long-term business resilience.


### 8. Executive Dashboard

![Final Dashboard](screenshots/dashboard.png)

Displays:

- Overall Crisis Score
- Leadership Score
- Negotiation Score
- Supply Chain Resilience
- Cost Control
- Risk Management
- Customer Satisfaction
- Biggest Mistake
- Best Decision
- Expert Recommendation
- Personalized Feedback


### Generated HTML File

[HTML FILE](https://github.com/ridak5845/60-days-claude-journey/blob/main/Day-29/operation-lifeline.html)

A complete self-contained HTML application built with React via CDN.

Runs completely offline by opening the HTML file in any modern browser.


### Sample Simulation Results

**Generated Company**

- Orion Supply Co
- Food & Beverage Processing
- Revenue: $668M
- 5 Factories
- 8 Warehouses
- 16 Suppliers


**Generated Crisis**

Supplier Bankruptcy

72% of critical components affected.


**Strategy Selected**

- Emergency Alternative Sourcing
- Proactive Customer Communication
- Inventory Rationing


**Negotiation Score**

75 / 100


**Leadership Score**

68 / 100


**Final Crisis Score**

77 / 100

**Rating**

Resilient Leader

### Key Learnings

#### Supply chains depend on preparation.

Organizations with diversified suppliers recover significantly faster than those relying on a single source.


#### Every decision has trade-offs.

Reducing cost often impacts customer satisfaction, while improving speed may reduce profitability.


#### Communication matters.

Keeping customers informed during disruptions protects trust and brand reputation.


#### AI improves resilience.

Predictive analytics, supplier monitoring, and procurement automation reduce future disruption risks.


#### Leadership extends beyond operations.

Effective crisis management requires balancing financial performance, operational continuity, employee coordination, and customer expectations.


### Outcome

This project demonstrates:

- React development
- UI/UX design
- Interactive simulation design
- Business strategy visualization
- Supply chain management concepts
- Enterprise dashboard development
- Decision-based game mechanics

---

#### Challenge

**Claude AI Challenge**

**Day 29 / 60**

**Project Completed Successfully**
