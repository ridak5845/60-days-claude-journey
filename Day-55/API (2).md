...
### `POST /review/file`
- **Purpose:** Review a single pasted/uploaded file's code.
- **Request body:** `{ filename, code }` *(JSON body today; multipart file upload UI comes Day 7 — endpoint already accepts raw code either way)*
- **Response:** Same shape as `/review/pr`.
